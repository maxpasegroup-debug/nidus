...
paymentsRouter.post("/create-order", ...financeRoles, [body("userId").optional().trim(), body("courseId").optional().trim(), body("amount").isFloat({ min: 1 }), body("currency").optional().trim().isLength({ min: 3, max: 3 }), body("paymentMethod").optional().trim(), body("product").optional().trim(), body("examSlug").optional().trim()], paymentsController.createOrder);
paymentsRouter.post("/orders", ...financeRoles, [body("userId").optional().trim(), body("courseId").optional().trim(), body("amount").isFloat({ min: 1 }), body("currency").optional().trim().isLength({ min: 3, max: 3 }), body("paymentMethod").optional().trim(), body("product").optional().trim(), body("examSlug").optional().trim()], paymentsController.createOrder);
