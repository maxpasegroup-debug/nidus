...
paymentsRouter.post("/orders", ...financeRoles, [body("userId").optional().trim(), body("courseId").optional().trim(), body("amount").isFloat({ min: 1 }), body("currency").optional().trim().isLength({ min: 3, max: 3 }), body("paymentMethod").optional().trim()], paymentsController.createOrder);
