...
export class DatabaseNdieQueueProvider implements NdieQueueProvider {
  ...
  async metrics(): Promise<NdieQueueMetrics> {
    ...
    return {
      ...
      aiValidationRunning: count("AI_VALIDATION_RUNNING"),
      aiValidationCompleted: count("AI_VALIDATION_COMPLETED"),
      readyForTeacherReview: count("READY_FOR_TEACHER_REVIEW"),
      readyForPublish: count("READY_FOR_PUBLISH"),
