...
const leadRoles = [protect, allowRoles(Role.ADMIN, Role.DIRECTOR, Role.ADMINISTRATIVE_OFFICER, Role.BUSINESS_DEVELOPMENT_EXECUTIVE, Role.TELECALLER, Role.MARKETING_COORDINATOR)];
...
crmRouter.post("/leads/bulk", ...bulkLeadRoles, [body("source").optional().trim(), body("notes").optional().trim(), body("leads").isArray({ min: 1, max: 500 }), body("leads.*.fullName").trim().notEmpty(), body("leads.*.mobile").trim().isLength({ min: 7 }), body("leads.*.email").optional({ values: "falsy" }).isEmail().normalizeEmail(), body("leads.*.targetExam").trim().notEmpty(), body("leads.*.source").optional().trim(), body("leads.*.notes").optional().trim()], crmController.createBulkLeads);
...
crmRouter.post("/followup", ...leadRoles, [body("leadId").notEmpty(), body("followUpDate").isISO8601(), body("remarks").trim().notEmpty(), body("status").trim().notEmpty()], crmController.createFollowUp);
crmRouter.get("/followups", ...leadRoles, crmController.followUps);

crmRouter.post("/admission", ...admissionRoles, [body("leadId").optional().trim(), body("studentId").notEmpty(), body("courseId").notEmpty(), body("instituteId").optional().trim(), body("branchId").optional().trim(), body("admissionDate").isISO8601(), body("paymentStatus").optional().trim(), body("batch").trim().notEmpty(), body("admissionMode").optional().isIn(["ONLINE", "MANUAL"]), body("totalFee").optional().isFloat({ min: 0 }), body("remarks").optional().trim()], crmController.createAdmission);
crmRouter.get("/admissions", ...admissionRoles, crmController.admissions);
...
crmRouter.post("/scholarships", ...admissionRoles, [body("studentId").notEmpty(), body("admissionId").optional().trim(), body("type").isIn(["SCHOLARSHIP", "DISCOUNT", "FEE_WAIVER"]), body("title").trim().notEmpty(), body("amount").isFloat({ min: 0 }), body("reason").optional().trim()], crmController.createScholarship);
...
crmRouter.post("/counselling", ...leadRoles, [body("leadId").notEmpty(), body("counsellorName").trim().notEmpty(), body("bookingDate").isISO8601(), body("mode").isIn(["ONLINE", "OFFLINE"]), body("status").trim().notEmpty()], crmController.createCounselling);
crmRouter.get("/counselling", ...leadRoles, crmController.counselling);

crmRouter.get("/referrals", ...admissionRoles, crmController.referrals);
crmRouter.post("/referrals", ...admissionRoles, [body("referrerUserId").notEmpty(), body("referredUserId").notEmpty(), body("rewardStatus").trim().notEmpty()], crmController.createReferral);
