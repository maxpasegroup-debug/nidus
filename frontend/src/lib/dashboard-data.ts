...
export function effectiveDashboardPath(user?: Pick<AuthUser, "role" | "roleMetadata"> | null) {
  ...
  if (template === "MARKETING" || template === "SALES_BOOSTER") return "/dashboard/marketing";
...
export function canAccessDashboardPath(user: Pick<AuthUser, "role" | "roleMetadata"> | null | undefined, path: string) {
  ...
  if (user.role === "TEACHER" && template === "ACADEMIC_HEAD") {
  ...
  if (template === "MARKETING" || template === "SALES_BOOSTER" || user.role === "MARKETING_COORDINATOR") {
    return path === "/dashboard/marketing" || path === "/dashboard/sales-booster";
  }
  if (user.role === "TELECALLER") return path === "/dashboard/telecaller";
  if (user.role === "STUDENT") return path === "/dashboard/student";
  if (user.role === "PARENT") return path === "/dashboard/parent";
  if (user.role === "GUEST") return path === "/dashboard/guest";
  return false;
}
