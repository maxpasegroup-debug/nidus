...
export function SimpleExamStudio({ initialTestId = "", initialStage = "home" }: { initialTestId?: string; initialStage?: Stage }) {
  ...
  async function uploadFile(event: ChangeEvent<HTMLInputElement>, kind: "QUESTION_PAPER" | "ANSWER_KEY") {
    ...
    if (!/\.(pdf|docx?)$/i.test(file.name)) { setError("Upload a PDF or Word document (.pdf, .doc, or .docx)."); return; }
    setBusy(true); setError("");
    try {
...
type EssentialsProps = {
  ...
  localStart: Date | null; localEnd: Date | null; busy: boolean; save: () => void; cancel: () => void;
};
...
function EssentialsStage(props: EssentialsProps) {
  ...
  return <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
    <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-sm"><div className="p-5 sm:p-6"><h2 className="text-xl font-black">Exam Essentials</h2><div className="mt-6 grid gap-x-5 gap-y-5 md:grid-cols-2">
      <TextField label="Exam name *" value={props.title} setValue={props.setTitle}/><SelectField label="Exam type *" value={props.examType} setValue={props.setExamType} options={["NDA", "CDS", "AFCAT", "AGNIVEER", "SSC"]}/>
      <TextField label="Subject *" value={props.subject} setValue={props.setSubject}/><TextField label="Topic *" value={props.topic} setValue={props.setTopic}/>
      <NumberField label="Duration *" value={props.duration} setValue={props.setDuration} suffix="minutes"/><NumberField label="Marks *" value={props.marks} setValue={props.setMarks}/>
      <NumberField label="Questions *" value={props.questionCount} setValue={props.setQuestionCount}/><label className="grid gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[var(--muted-blue)]">Short note<textarea value={props.note} onChange={(event) => props.setNote(event.target.value)} rows={2} className="min-h-12 rounded-xl border border-[var(--border)] bg-white px-3 py-3 text-sm font-semibold normal-case tracking-normal text-[var(--navy)] outline-none focus:border-[var(--gold)]"/></label>
      <label className="grid gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[var(--muted-blue)]">Exam start date *<input type="date" value={props.startDate} onChange={(event) => props.setStartDate(event.target.value)} className={fieldClass}/></label><label className="grid gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[var(--muted-blue)]">Exam start time *<input type="time" value={props.startTime} onChange={(event) => props.setStartTime(event.target.value)} className={fieldClass}/></label>
...
function SelectField({ label, value, setValue, options }: { label: string; value: string; setValue: (value: string) => void; options: string[] }) { return <label className="grid gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[var(--muted-blue)]">{label}<select value={value} onChange={(event) => setValue(event.target.value)} className={fieldClass}>{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
function TextField({ label, value, setValue }: { label: string; value: string; setValue: (value: string) => void }) { return <label className="grid gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[var(--muted-blue)]">{label}<input value={value} onChange={(event) => setValue(event.target.value)} className={fieldClass} /></label>; }
function NumberField({ label, value, setValue, suffix }: { label: string; value: number; setValue: (value: number) => void; suffix?: string }) { return <label className="grid gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[var(--muted-blue)]">{label}<span className="flex overflow-hidden rounded-xl border border-[var(--border)] bg-white focus-within:border-[var(--gold)]"><input type="number" min={1} value={value} onChange={(event) => setValue(Number(event.target.value))} className="h-12 min-w-0 flex-1 px-3 text-sm font-semibold normal-case tracking-normal outline-none"/>{suffix ? <span className="grid place-items-center border-l border-[var(--border)] px-4 text-xs font-semibold normal-case tracking-normal text-[var(--muted-blue)]">{suffix}</span> : null}</span></label>; }
function UploadField({ label, file, onChange }: { label: string; file?: UploadRecord; onChange: (event: ChangeEvent<HTMLInputElement>) => void }) { return <label className="flex min-h-28 cursor-pointer items-center gap-3 rounded-xl border-2 border-dashed border-[var(--border)] bg-white p-4 transition hover:border-[var(--gold)] hover:bg-[var(--gold-soft)]"><Upload className="text-[var(--gold)]"/><span><b>{file ? "File selected" : label}</b><small className="block text-[var(--muted-blue)]">{file?.originalName || "PDF or Word (.pdf, .doc, .docx)"}</small></span><input className="sr-only" type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={onChange} /></label>; }
