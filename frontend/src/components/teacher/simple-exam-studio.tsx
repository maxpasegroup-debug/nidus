...
export function SimpleExamStudio({ initialTestId = "", initialStage = "home" }: { initialTestId?: string; initialStage?: Stage }) {
  ...
  async function submitRelease(requestedAction?: "SAVE_DRAFT" | "SCHEDULE" | "PUBLISH_NOW") {
    // React click handlers receive a SyntheticEvent argument. Accept only the
    // explicit release-action strings so a DOM/Fiber event can never enter the
    // JSON request body when this function is used as a button callback.
    const action = typeof requestedAction === "string" ? requestedAction : releaseChoice;
...
function reviewIssueCopy(issue: ReviewIssue) {
  switch (issue.id || issue.type) {
    ...
    case "MISSING_REQUIRED_VISUAL":
      return {
        title: "Required visual is missing",
        detail: "Attach or confirm the diagram, graph, table, or figure referenced by this question before releasing.",
      };
    case "VISUAL_SOURCE_NEEDS_REVIEW":
      return {
...
function formatReviewMath(value: string) {
  return value.replace(/\[([^\[\]\n]*;[^\[\]\n]*)\]/g, (_, matrix: string) => {
    const rows = matrix.split(";").map((row: string) => row.trim().split(/\s+/).join(" & "));
    return `$\\begin{bmatrix}${rows.join("\\\\")}\\end{bmatrix}$`;
  });
}
function canonicalBlocks(value: unknown) { const blocks = value && typeof value === "object" && !Array.isArray(value) ? (value as { blocks?: unknown }).blocks : undefined; return Array.isArray(blocks) ? blocks.filter((block): block is { type?: string; text?: string; segments?: NidusRichSegment[]; options?: Array<{ key?: string; text?: string; segments?: NidusRichSegment[] }>; assetUrl?: string; pageNumber?: number } => Boolean(block && typeof block === "object" && !Array.isArray(block))) : []; }
function QuestionPreview({ question }: { question: Question }) { const blocks = canonicalBlocks(question.contentJson); const paragraph = blocks.find((block) => block.type === "paragraph"); const optionBlock = blocks.find((block) => block.type === "options"); const explanation = blocks.find((block) => block.type === "explanation"); const visuals = blocks.filter((block) => block.type === "visual"); const options = optionBlock?.options || []; return <div><p className="mt-3 font-semibold">{paragraph?.segments?.length ? <NidusRichSegments segments={paragraph.segments} /> : <NidusMathText text={formatReviewMath(paragraph?.text || question.questionText || "Question text missing")} />}</p>{visuals.map((visual, index) => visual.assetUrl ? <img key={index} src={visual.assetUrl} alt="Question visual" className="mt-3 max-h-80 max-w-full rounded border object-contain" /> : <p key={index} className="mt-3 rounded border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-900">Visual source from page {visual.pageNumber || "?"} is preserved for review.</p>)}<div className="mt-2 grid gap-2 text-sm md:grid-cols-2">{["A", "B", "C", "D"].map((key, index) => { const rich = options.find((option) => option.key === key); const value = rich?.text || [question.optionA, question.optionB, question.optionC, question.optionD][index] || "Missing"; return <p key={index} className="rounded bg-slate-50 p-2">{key}. {rich?.segments?.length ? <NidusRichSegments segments={rich.segments} /> : <NidusMathText text={formatReviewMath(value)} />}</p>; })}</div><p className="mt-2 text-xs">Answer: {question.correctAnswer || "To be completed"} · {question.marks} marks</p>{explanation?.segments?.length ? <p className="mt-1 text-xs"><NidusRichSegments segments={explanation.segments} /></p> : !question.explanation.trim() ? <p className="mt-1 text-xs text-slate-500">Explanation optional.</p> : null}</div>; }
