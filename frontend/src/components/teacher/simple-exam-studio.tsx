...
function QuestionPreview({ question }: { question: Question }) { return <div><p className="mt-3 font-semibold"><NidusMathText text={formatReviewMath(question.questionText || "Question text missing")} /></p><div className="mt-2 grid gap-2 text-sm md:grid-cols-2">{[question.optionA, question.optionB, question.optionC, question.optionD].map((option, index) => <p key={index} className="rounded bg-slate-50 p-2">{String.fromCharCode(65 + index)}. <NidusMathText text={formatReviewMath(option || "Missing")} /></p>)}</div><p className="mt-2 text-xs">Answer: {question.correctAnswer || "To be completed"} · {question.marks} marks</p>{!question.explanation.trim() ? <p className="mt-1 text-xs text-slate-500">Explanation optional.</p> : null}</div>; }
