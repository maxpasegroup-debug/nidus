...
export function ProgramEnquiryForm({ programTitle, source = "Academy Program Page" }: { programTitle: string; source?: string }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-[#263a8f]/10 bg-white/86 p-5 shadow-[0_24px_80px_rgba(19,35,72,0.12)] backdrop-blur-2xl">
      <div className="flex items-start gap-3">
        <div className="grid h-11 w-11 place-items-center rounded bg-[#263a8f]/10 text-[#263a8f]">
          <ShieldCheck className="h-5 w-5" />
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#263a8f]">Enquire Now</p>
          <h2 className="mt-1 text-2xl font-semibold text-[#111827]">{programTitle}</h2>
          <p className="mt-2 text-sm leading-6 text-[#536072]">Your enquiry will be saved to the NIDUS lead management dashboard.</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-semibold text-[#111827] sm:col-span-2">
          Message
          <textarea value={form.message} onChange={(event) => update("message", event.target.value)} className="min-h-28 rounded border border-[#263a8f]/15 bg-white px-3 py-3 text-sm font-medium text-[#111827] outline-none focus:border-[#263a8f]" placeholder="Tell us your goal, current class, preferred batch, or counselling question." />
        </label>
      </div>
