...
import { GuestApplicantDashboard, type GuestApplicantView } from "@/components/dashboard/guest-applicant-dashboard";
...
export function GuestApplicantRoute({ view }: { view: GuestApplicantView }) {
  const { user } = useAuth();
  ...
  return <GuestApplicantDashboard name={user?.name} view={view} />;
}
