...
export function DirectorLauncher({
  ...
  return (
    <main className="relative flex min-h-[calc(100vh-var(--nav-height)-2rem)] flex-col bg-[var(--page-bg)] text-[var(--navy)] lg:h-[calc(100vh-var(--nav-height)-2rem)] lg:overflow-hidden">
      <section className="mx-auto flex w-full max-w-[1500px] flex-1 flex-col gap-4 px-1 pb-24 lg:px-2 lg:pb-4">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            {stats?.length ? (
              <div className="grid gap-2 sm:grid-cols-3 lg:w-[600px]">
                {stats.map((stat) => (
                  <div key={stat.label} className="rounded-xl border border-[var(--border)] bg-[var(--page-bg)] px-4 py-2">
        ...
        <section className="grid flex-1 auto-rows-fr gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {tiles.map((tile) => (
...
function DirectorActionTile({ tile }: { tile: DirectorTile }) {
  ...
  const className = `relative flex min-h-36 flex-col justify-between rounded-2xl border p-5 text-left shadow-sm transition lg:min-h-0 ${
    tile.muted
  ...
  const content = (
    <>
      {tile.badge ? <span className="absolute right-4 top-4 rounded-full bg-red-100 px-2.5 py-1 text-xs font-black text-red-700">{tile.badge}</span> : null}
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--page-bg)]">
        <Icon className="h-6 w-6" />
      </div>
      <div>
        <h2 className="text-lg font-black leading-tight">{tile.label}</h2>
        {tile.note ? <p className="mt-2 text-sm leading-6 text-[var(--muted-blue)]">{tile.note}</p> : null}
