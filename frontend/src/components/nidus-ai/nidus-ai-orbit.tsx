...
export function NidusAiOrbit({ message, mood = "guide" }: { message: string; mood?: NidusAiMood }) {
  return (
    <div className="relative overflow-hidden rounded-lg border border-gold/20 bg-white/[0.055] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.18)] backdrop-blur-xl">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">{moodCopy[mood]}</p>
          <p className="mt-3 text-lg font-semibold leading-7 text-white">{message}</p>
        </div>
      </div>
    </div>
  );
