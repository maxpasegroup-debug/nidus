  ...
  return (
    <div className="bg-[#f7f3ea] pt-20 text-[#101827]">
      <section className="relative min-h-[calc(100vh-5rem)] overflow-hidden px-4 py-16 sm:px-6 lg:px-8">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_22%_16%,rgba(185,145,63,0.15),transparent_28rem),radial-gradient(circle_at_76%_18%,rgba(110,143,175,0.18),transparent_24rem),linear-gradient(180deg,#fffdf8_0%,#f7f3ea_100%)]" />
        <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#3f4a32]">Join NIDUS</p>
            <h1 className="mt-5 text-4xl font-semibold leading-[0.98] text-[#071d36] sm:text-7xl">Meet your NIDUS AI Assistant.</h1>
            <p className="mt-6 max-w-xl text-base leading-8 text-[#64748b] sm:text-lg">Share a few basic details. The assistant will prepare your application message and connect you to NIDUS Academy on WhatsApp for immediate support.</p>
            ...
            <div className="mt-8 grid gap-3">
              {assistantSteps.map((step, index) => {
                ...
                return (
                  <div key={step} className={`flex items-center gap-3 rounded-lg border px-4 py-3 text-sm font-semibold ${active ? "border-[#071d36]/18 bg-white/75 text-[#071d36]" : "border-[#071d36]/8 bg-white/40 text-[#64748b]"}`}>
                    <span className={`grid h-7 w-7 place-items-center rounded-full ${active ? "bg-[#071d36] text-white" : "bg-[#071d36]/7 text-[#071d36]"}`}>{active ? <CheckCircle2 className="h-4 w-4" /> : index + 1}</span>
                    {step}
                  </div>
                );
              })}
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65 }} className="rounded-[1.5rem] border border-[#071d36]/10 bg-white/84 p-5 shadow-[0_28px_90px_rgba(7,29,54,0.12)] backdrop-blur-2xl sm:p-7">
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#f7f3ea] text-[#3f4a32]">
                <Sparkles className="h-6 w-6" />
              </div>
              <div>
                <h2 className="text-2xl font-semibold text-[#071d36]">Application Assistant</h2>
                <p className="text-sm text-[#64748b]">Basic information only. WhatsApp support follows next.</p>
              </div>
            </div>
            ...
            <form onSubmit={onSubmit} className="mt-7 grid gap-4">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="grid gap-2 text-sm font-semibold text-[#071d36]">
                  Interested program
                  <select value={form.program} onChange={(event) => updateField("program", event.target.value)} className="h-12 rounded border border-[#071d36]/14 bg-white px-3 text-sm font-medium text-[#101827] outline-none focus:border-[#3f4a32]">
              ...
              {submitted ? (
                <div className="rounded-lg border border-[#25d366]/25 bg-[#25d366]/8 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-[#071d36]">Thank you. I will connect you through WhatsApp now.</h3>
                      <p className="mt-1 text-sm leading-6 text-[#64748b]">Your greeting and application message is ready for NIDUS Academy support.</p>
                    </div>
                  </div>
                  ...
                  <button type="button" onClick={() => setSubmitted(false)} className="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded border border-[#071d36]/14 bg-white px-4 py-2 text-sm font-semibold text-[#071d36] transition hover:border-[#b9913f]/60 sm:ml-3 sm:mt-4 sm:w-auto">
                    Edit Details
                  </button>
                </div>
              ) : (
            ...
            <div className="mt-6 rounded-lg border border-[#071d36]/10 bg-[#f7f3ea] p-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-[#071d36]">
                <ClipboardList className="h-4 w-4 text-[#3f4a32]" />
                WhatsApp message preview
              </div>
              <pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap rounded border border-[#071d36]/8 bg-white p-3 text-xs leading-6 text-[#64748b]">{whatsappMessage}</pre>
            </div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              {[
                ...
                return (
                  <div key={String(label)} className="rounded-lg border border-[#071d36]/10 bg-[#f7f3ea] p-3 text-sm font-semibold text-[#071d36]">
...
function Field({ label, value, onChange, required = false, className = "", autoComplete, inputMode, type = "text" }: { label: string; value: string; onChange: (value: string) => void; required?: boolean; className?: string; autoComplete?: string; inputMode?: "text" | "tel"; type?: string }) {
  return (
    <label className={`grid gap-2 text-sm font-semibold text-[#071d36] ${className}`}>
      {label}
      <input type={type} required={required} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} inputMode={inputMode} className="h-12 rounded border border-[#071d36]/14 bg-white px-3 text-sm font-medium text-[#101827] outline-none focus:border-[#3f4a32]" />
    </label>
  );
}
